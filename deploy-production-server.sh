#!/bin/bash
# skaptix-website Docker Deployment Script (Production Environment)
# Usage: ./deploy-production-server.sh -ServerIP <server_ip> [-ServerPassword <password> | -PemFile <path>] -SSHUser <username>
...
# Load build arguments from .env.prod
if [ -f ".env.prod" ]; then
    echo "📄 Loading .env.prod for build arguments..."
...
if [ -n "$SERVER_PASSWORD" ]; then
    $SSH_CMD "$SSH_HOST" "printf '%s\n' '${SERVER_PASSWORD}' | sudo -S sh -c 'mkdir -p /data-drive/tmp && chown ${SSH_USER}:${SSH_USER} /data-drive/tmp' 2>/dev/null"
else
    $SSH_CMD "$SSH_HOST" "sudo sh -c 'mkdir -p /data-drive/tmp && chown ${SSH_USER}:${SSH_USER} /data-drive/tmp'"
fi

if ! $SCP_CMD "${APP_NAME}-${IMAGE_TAG}.tar" "${SSH_HOST}:/data-drive/tmp/" ; then
...
$SSH_CMD "$SSH_HOST" "mkdir -p /data-drive/skaptix/apps/${APP_NAME}"
$SCP_CMD ".env.prod" "${SSH_HOST}:/data-drive/skaptix/apps/${APP_NAME}/.env.prod"
$SCP_CMD "docker-compose.yml" "${SSH_HOST}:/data-drive/skaptix/apps/${APP_NAME}/docker-compose.yml"
...
echo 'Cleaning up...'
rm -f /data-drive/tmp/APP_NAME-IMAGE_TAG.tar
...
$SCP_CMD deploy_remote.sh "${SSH_HOST}:/data-drive/tmp/deploy.sh"

echo "Executing remote deployment script..."
if ! $SSH_CMD "$SSH_HOST" "chmod +x /data-drive/tmp/deploy.sh && /data-drive/tmp/deploy.sh"; then
...
rm -f "${APP_NAME}-${IMAGE_TAG}.tar"
rm -f deploy_remote.sh
chmod +x deploy-production-server.sh 2>/dev/null || true
